...
exports.formatSajuData = formatSajuData;
const lunisolar_1 = __importDefault(require("lunisolar"));
// 천간 10개
 ...
 */
function findSolarTerm(year, month, day, hour = 0, minute = 0) {
    try {
        ...
        if (!foundTerm) {
            try {
                ...
                const termDateArray = lunisolar_1.default.SolarTerm.findDate(year - 1, lastTerm);
                if (termDateArray && termDateArray.length >= 3) {
                    const termDate = new Date(termDateArray[0], termDateArray[1] - 1, termDateArray[2], 0, 0);
 ...
 * 음력인 경우 양력으로 변환 후 계산
 */
function calculateYearPillar(birth) {
    // 음력이면 양력으로 변환
 ...
 * 정확한 절기 계산 사용
 */
function calculateMonthPillar(birth, yearStem) {
    // 음력이면 양력으로 변환
    const solar = convertLunarToSolar(birth);
    // 월간 결정표 (연간 기준)
 ...
 * 분 단위까지 고려
 */
function calculateDayPillar(birth) {
    // 음력이면 양력으로 변환
    const solar = convertLunarToSolar(birth);
 ...
 * 일간 기준으로 시간의 천간 결정
 */
function calculateHourPillar(birth, dayStem) {
    // 시간대별 지지 (2시간 단위)
...
//# sourceMappingURL=saju_calculation.js.map