...
const metrics_extended_1 = require("./metrics_extended");
    ...
    try {
        ...
        if (args.file) {
            ...
            try {
                ...
                calculatedMetrics = (0, metrics_1.calculateMetrics)(data);
                // 확장 메트릭 계산 (삼정/오악/조합표용)
                const extendedMetrics = (0, metrics_extended_1.calculateExtendedMetrics)(calculatedMetrics, data);
                // 색상 데이터 추출 (이미지가 있으면)
                let colorDataText = "";
                if (args.image) {
                    console.log(`🎨 이미지에서 색상 데이터 추출 중: ${args.image}`);
                    const landmarks = data.faces?.[0]?.landmarks || data.landmarks || [];
                    const colorData = await (0, color_1.extractColorData)(args.image, landmarks);
                    colorDataText = "\n\n" + (0, color_1.formatColorDataForPrompt)(colorData);
                }
                metricsText = (0, metrics_1.formatMetricsForPrompt)(calculatedMetrics) + colorDataText;
            ...
            try {
                ...
                throw new Error("❌ --metrics JSON 파싱 실패. 올바른 JSON 형식인지 확인하세요.");
            }
        }
        else {
            console.log("\n사용법:");
        ...
        if (args.query) {
            ...
            searchQuery = (0, metrics_1.summarizeMetricsForSearch)(calculatedMetrics);
        }
        else {
            searchQuery = "관상 분석 성격 재물운 직업운 애정운 건강운";
        }
        console.log(`🔍 검색 쿼리: "${searchQuery.slice(0, 50)}..."`);
        const relevantChunks = (0, rag_1.searchChunks)(chunks, searchQuery, 5);
        console.log(`📑 ${relevantChunks.length}개의 관련 문서 찾음`);
        const context = (0, rag_1.formatContext)(relevantChunks);
        // 3. 프롬프트 생성
        const systemPrompt = (0, prompt_1.buildSystemPrompt)(context);
        const userPrompt = (0, prompt_1.buildUserPrompt)(metricsText, args.query);
        // 4. API 호출
        const result = await (0, gms_1.callGmsApi)(systemPrompt, userPrompt);
        // 5. 결과 출력
        console.log("\n" + "═".repeat(55));
