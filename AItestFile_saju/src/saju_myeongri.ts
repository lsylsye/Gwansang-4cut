...
export interface GanjiRelation {
  // 천간 합
  ganCombinations: Array<{ gan1: string; gan2: string; result: string }>;
...
export interface MyeongriData {
  ...
  sipSung: SipSungData;
  twelveFortune: Record<string, string>; // 지지별 12운성
 ...
 * 오행 분포 계산
 */
export function calculateFiveElements(saju: SajuData): FiveElementDistribution {
  ...
  stems.forEach(stem => {
    ...
    if (element) distribution[element as keyof FiveElementDistribution]++;
  });
 ...
 * 일간을 기준으로 다른 간지와의 관계
 */
export function calculateSipSung(saju: SajuData): SipSungData {
  const dayStem = saju.dayStem;
  const dayElement = stemFiveElements[dayStem];
  const dayYinYang = stemYinYang[dayStem];
  
  // 십성 매핑 함수
  function getSipSung(targetStem: string, targetBranch: string): string {
    ...
    // 나를 극하는 오행 (정관/편관)
    const reverseOvercome: Record<string, string> = {
      '토': '목', '금': '화', '수': '토', '목': '금', '화': '수'
    };
 ...
 * 일간 기준으로 각 지지의 운성
 */
export function calculateTwelveFortune(dayStem: string): Record<string, string> {
  // 일간별 12운성 순서
 ...
 * 천간 합 계산
 */
function calculateGanCombinations(saju: SajuData): Array<{ gan1: string; gan2: string; result: string }> {
  const combinations: Array<{ gan1: string; gan2: string; result: string }> = [];
  
  // 천간 5합
  const ganCombinations: Record<string, { partner: string; result: string }> = {
 ...
 * 지지 관계 계산 (합, 충, 형, 해, 파)
 */
function calculateBranchRelations(saju: SajuData): Array<{
  type: '합' | '충' | '형' | '해' | '파';
  branch1: string;
  branch2: string;
  description: string;
  ...
  // 3합
  const threeCombinations: string[][] = [
    ['인', '오', '술'], // 화국
    ['해', '묘', '미'], // 목국
    ['사', '유', '축'], // 금국
    ['신', '자', '진']  // 수국
 ...
 * 전체 명리 데이터 계산
 */
export function calculateMyeongri(saju: SajuData): MyeongriData {
 ...
 * 명리 데이터를 텍스트로 포맷팅
 */
export function formatMyeongriData(myeongri: MyeongriData): string {
  let text = `## 명리 관계 분석
