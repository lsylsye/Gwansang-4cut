...
export interface ColorData {
  // 눈 기색
  sclera_redness?: number; // 눈 흰자 붉음 (R-G 차이)
  eye_region_rgb?: { r: number; g: number; b: number };

  // 코 기색
  nose_tip_redness?: number; // 코끝 붉음
  nose_tip_rgb?: { r: number; g: number; b: number };

  // 입 기색
  lip_redness?: number; // 입술 붉음/검푸름 (Hue/Sat 기반)
  lip_region_rgb?: { r: number; g: number; b: number };

  // 피부 (점/주름은 텍스처 분석 필요 - 여기선 기본 RGB만)
  temple_spot_score?: number; // 관자 잡티 (명도 차이)
  forehead_wrinkle_score?: number; // 이마 주름 (명도 분산)
}

/**
 * 이미지 로드 및 픽셀 추출 (sharp 사용)
 */
async function loadImagePixels(imagePath: string): Promise<{
  width: number;
  height: number;
  data: Buffer;
}> {
 ...
 */
export async function extractColorData(
  imagePath: string,
  landmarks: Landmark[]
): Promise<ColorData> {
  // 상대 경로를 절대 경로로 변환
 ...
 * 색상 데이터를 텍스트로 포맷팅
 */
export function formatColorDataForPrompt(colorData: ColorData): string {
