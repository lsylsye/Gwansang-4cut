 ...
 * 두 점 사이 각도 계산 (도 단위)
 */
function angleDeg(p1: Landmark, p2: Landmark): number {
  const dx = p2.x - p1.x;
  const dy = p2.y - p1.y;
 ...
 */
export function calculateMetrics(data: MediaPipeFaceData): FaceMetricsCalculated {
  // 랜드마크 추출
 ...
 */
export function formatMetricsForPrompt(m: FaceMetricsCalculated): string {
 ...
 */
export function summarizeMetricsForSearch(m: FaceMetricsCalculated): string {
  const parts: string[] = ["관상 분석"];

  // 얼굴형
  if (m.face_ratio_wh < 0.75) parts.push("긴 얼굴형");
  else if (m.face_ratio_wh > 0.85) parts.push("넓은 얼굴형");
  else parts.push("균형 얼굴형");

  // 이마
  if (m.forehead_height_ratio > 0.15) parts.push("높은 이마");
  else if (m.forehead_height_ratio < 0.12) parts.push("낮은 이마");

  // 눈
  if (m.eye_asymmetry > 0.003) parts.push("눈 비대칭");
  if (m.inter_eye_dist > 0.17) parts.push("넓은 미간");

  // 입
  if (m.mouth_corner_slope < -3) parts.push("처진 입꼬리");
  else if (m.mouth_corner_slope > 3) parts.push("올라간 입꼬리");

  // 턱
  if (m.jaw_angle < 65) parts.push("각진 턱");
  else if (m.jaw_angle > 75) parts.push("둥근 턱");
