...
export async function analyzeFaceFirstInitial(
  ...
  try {
    ...
    devError('❌ 총평(first-initial) API 오류:', error);
    return {
      stage1: null,
      totalReview: null,
      error: error instanceof Error ? error.message : '알 수 없는 오류',
...
export async function analyzeFaceFirstRemaining(
  ...
  try {
    ...
    devError('❌ 나머지 관상(first-remaining) API 오류:', error);
    return {
      success: false,
      totalReview: {},
      error: error instanceof Error ? error.message : '알 수 없는 오류',
...
export async function analyzeFaceSecond(
  ...
  try {
    ...
    devError('❌ 체질/메뉴 2차 API 오류:', error);
...
export async function analyzeGroupOverall(payload: {
  ...
  try {
    ...
    devError('❌ 모임 전체 궁합 API 오류:', error);
    return {
      success: false,
      error: error instanceof Error ? error.message : '알 수 없는 오류',
    };
...
export async function analyzeGroupPairs(payload: {
  ...
  try {
    ...
    devError('❌ 모임 1:1 궁합 API 오류:', error);
    return {
      success: false,
      error: error instanceof Error ? error.message : '알 수 없는 오류',
    };
